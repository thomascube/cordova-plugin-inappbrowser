...
- (UILabel*)buildAddressLabel
{
    ...
    if ([addressLabel respondsToSelector:NSSelectorFromString(@"setMinimumScaleFactor:")]) {
        [addressLabel setValue:@(10.0/[UIFont labelFontSize]) forKey:@"minimumScaleFactor"];
    } else if ([addressLabel respondsToSelector:NSSelectorFromString(@"setMinimumFontSize:")]) {
...
- (UIImage*)drawArrow:(BOOL)backwards
{
    ...
    CGContextAddLineToPoint(context, canvasSize.width * left/10.0, canvasSize.height * 7.2/10.0);
...
{
    return [self drawArrow:NO];
}

-(UIImage*)drawBackArrow
{
    return [self drawArrow:YES];
}
...
- (void) rePositionViews {
    if ([_browserOptions.toolbarposition isEqualToString:kInAppBrowserToolbarBarPositionTop]) {
        ...
        [self.webView setFrame:CGRectMake(self.webView.frame.origin.x, self.toolbar.frame.origin.y + self.toolbar.frame.size.height, self.webView.frame.size.width, self.webView.frame.size.height)];
    }
}
